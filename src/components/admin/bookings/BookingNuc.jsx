...
import { BookAl } from './BookingNucComp'
...
const BookingNuc = () => {
    const [Table, setTable] = useState({ Table: BookAl });
    const [active, setActive] = useState(1)
    return (
        <>
            <Header title="New & Used Cars" navbar_comp={<CommonHeader />} />
                ...
                <div className="page_reducer">
                    ...
                    <div className="cons_content_2 mt-2">
                        <div className="cons_cate d-flex">
                            <div className={active === 1 ? "active_link" : "unactive_link"} onClick={() => { setTable({ Table: BookAl }); setActive(1) }}>All Listings (192)</div>
                            <div className={`ml-3 ${active === 2 ? "active_link" : "unactive_link"}`} onClick={() => { setTable({ Table: BookAl }); setActive(2) }}>Listed by Dealers (92)</div>
                            <div className={`ml-3 ${active === 3 ? "active_link" : "unactive_link"}`} onClick={() => { setTable({ Table: BookAl }); setActive(3) }}>Listed by Individuals (80)</div>
                            <div className={`ml-3 ${active === 4 ? "active_link" : "unactive_link"}`} onClick={() => { setTable({ Table: BookAl }); setActive(4) }}>MG Certified Cars (192)</div>
